...
class TestsStats(Resource):
	...
	def get(self):
		...
		try:
			...
			total_tests = db[collection1].find({}).count()
			deleted_tests = db[collection1].find({"deleted": 1}).count()
			tests = {
				'total': total_tests,
				'deleted': deleted_tests,
				'active': total_tests - deleted_tests
			}

			columns = {'_id': 1}
			queries = {}
			tests_ids = FlaskMongo.find(collection1, columns, queries)
			tests_ids = [tid.get('_id') for tid in tests_ids]
