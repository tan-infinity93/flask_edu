...
class RoomsEnrolledStats(Resource):
	'''
	'''
	def __init__(self):
		'''
	...
	def get(self):
		...
		try:
			...
			banned_rooms_enroll = db[collection1].find({"banned": True}).count()
			rooms_enroll = {
			...
			columns = {'_id': 0, 'room_id': 1, 'limit': 1}
			queries = {}
			rooms_enroll_counts = FlaskMongo.find(collection1, columns, queries)
